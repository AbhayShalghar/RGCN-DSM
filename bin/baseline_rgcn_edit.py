...
def func():
    ...
    k = 1
    model = Model(114, 20, 1, hetero_graph.etypes)
    ...
    for epoch in range(1):
        negative_graph = construct_negative_graph(hetero_graph, k, ('Person', 'partner', 'Person'))
        #print((negative_graph))
        #nx.draw(negative_graph.to_networkx())
        #plt.show()
        #plt.plot(scalex=1000, scaley=1000, data=hetero_graph)
        #plt.show()
        pos_score, neg_score = model(hetero_graph, negative_graph, node_features, ('Person', 'partner', 'Person'))
