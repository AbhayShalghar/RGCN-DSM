...
def func():
    ...
    #for i in range(100):
    #   print(dsm[i])
    #print(graph_data[('Person', 'partner', 'Person')][0][0].item(),graph_data[('Person', 'partner', 'Person')][1][0].item())
    etypes_tensor = []
    for i in range(0,hetero_graph.number_of_edges('student')):
        for j in dsm:
            if (j['entity1'] == graph_data[('Person', 'student', 'Person')][0][i].item()) and (j['entity2'] == graph_data[('Person', 'student', 'Person')][1][i].item()):
                etypes_tensor.append(j['score'])
                break
            else:
                etypes_tensor.append(0)
        
    etypes_tensor = etypes_tensor[0:hetero_graph.number_of_edges('student')]
    print(len(etypes_tensor))
    import torch
    ...
    #print(type(hetero_graph.nodes['Person'].data['Person']))
    #print(hetero_graph.number_of_edges('student'))
    ...
    class RGCN(nn.Module):
        ...
        def forward(self, graph, inputs):
            ...
            h = self.conv1(graph, inputs ,{'edge_weight':x})
            h = {k: F.relu(v) for k, v in h.items()}
            h = self.conv2(graph, h,{'edge_weight':x})
            return h
    ...
    #print(hetero_graph.edges(etype='student'))
