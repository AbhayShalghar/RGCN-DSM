...
class RGCN(nn.Module):
    ...
    def forward(self, graph, inputs):
        ...
        #h1 where we have to multiply dsm
        h = h['Person'] #h(i)
        h_iota = h.clone()
            #print(h_iota)
        ...
        for doc in dsm:
            ...
            try:
                ds = th.matmul(h[index_1], (torch.FloatTensor(add_up)))
                h_iota[index_1] = h[index_1] + torch.sigmoid(ds)
                ds_1 = th.matmul(h[index_2], (torch.FloatTensor(add_up)))
                h_iota[index_2] = h[index_2] + torch.sigmoid(ds_1)
            except:
                pass
        f = {'Person':h_iota}

        h = {k: F.relu(v) for k, v in f.items()}
        h = self.conv3(graph, h)
        #h2 where we have to multiply dsm again
        return h
    ...
    def forward(self, graph, h, etype):
        ...
        with graph.local_scope():
            ...
            for doc in dsm:
                ...
                #add_up = [sum(doc['tensor'])]
                try:
                    sig_1 = th.sigmoid(h[index_1])
                    ds = th.matmul(h[index_1], (torch.FloatTensor(add_up)))
                    if sig_1 > 0.5:
                        #h_iota[index_1] = h[index_1] - torch.sigmoid(ds)
                        h_iota[index_1] = h[index_1]
                    else:
                        h_iota[index_1] = h[index_1] + torch.sigmoid(ds)
                    sig_2 = th.sigmoid(h[index_2])
                    ds_1 = th.matmul(h[index_2], (torch.FloatTensor(add_up)))
                    if sig_2 > 0.5:
                        #h_iota[index_2] = h[index_2] - torch.sigmoid(ds_1)
                        h_iota[index_2] = h[index_2]
                    else:
                        h_iota[index_2] = h[index_2] + torch.sigmoid(ds_1)
                        #check for convegence
                        #how much is negtive conttributing
                        #only positive 
                        #proportional method
                except:
                    pass
...
for i in range(2):
    for epoch in range(25):
        negative_graph = construct_negative_graph(hetero_graph, k, ('Person', 'child', 'Person'))
        #print((negative_graph))
        #nx.draw(negative_graph.to_networkx())
        #plt.show()
        #plt.plot(scalex=1000, scaley=1000, data=hetero_graph)
        #plt.show()
        pos_score, neg_score = model(hetero_graph, negative_graph, node_features, ('Person', 'child', 'Person'))
