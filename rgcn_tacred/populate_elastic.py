...
def publish(input_file):
    ...
    for line in lines:
        ...
        try:
            ...
            doc['text'] = jd['title']
            doc['type'] = "title"
            res = es.index(index="tacred-test", id= doc['qid'] + "_1", body=doc)
        ...
        for t in tokens:
            try:
                ...
                res = es.index(index="tacred-test", id=doc['qid'] + "_text_" + str(n), body=doc)
                n = n + 1
            except Exception as e:
        ...
        try:
            ...
            res = es.index(index="tacred-test", id=doc['qid'] + "_3", body=doc)
        except Exception as e:
        ...
        try:
            if jd.get("infobox") is not None and len(jd.get("infobox")) > 0:
                ...
                for k, v in jd['infobox'].items():
                    ...
                    doc["text"] = k
                    doc['type'] = "infobox"
                    res = es.index(index="tacred-test", id=doc['qid'] +  "_info_" + str(y), body=doc)
                    y = y + 1
        except Exception as e:
        ...
        for section_by_title in section_by_title_list:
            if section_by_title.get("text") is not None:
                try:
                    ...
                    doc['type'] = "section"
                    res = es.index(index="tacred-test", id=doc['qid'] + "_5", body=doc)
                except Exception as e:
                    ...
                    for subsection_map in subsections_list:
                        if subsection_map.get('text') is not None:
                            try:
                                ...
                                doc['type'] = "subsection"
                                res = es.index(index="tacred-test", id=doc['qid'] + "_6", body=doc)
                            except Exception as e:
                                print("Error writing subsection")
                                continue

input_file_1 = "tacred_wikipedia_pages.json.5"
input_file_2 = "tacred_wikipedia_pages.json.6"
input_file_3 = "tacred_wikipedia_pages.json.7"
input_file_4 = "tacred_wikipedia_pages.json.8"
input_file_5 = "tacred_wikipedia_pages.json.9"
